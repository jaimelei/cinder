    ...
    return (
        ...
        >
            <div className="relative overflow-hidden">
                <img
                    src={video.thumbnail_url || getThumbnailUrl(video.youtube_id, "hq")}
                    alt={video.title}
                    loading="lazy"
                    onError={(e) => {
                        e.currentTarget.src = getThumbnailUrl(video.youtube_id, "sd");
                    }}
                    className="
                        aspect-video
                        w-full
                ...
                {video.duration && (
                    <div className="absolute bottom-2 right-2 rounded bg-charcoal-950/80 px-1.5 py-0.5 text-[11px] text-ash-50 backdrop-blur-sm">
            ...
            <div className="space-y-2 p-3">
                <h3 className="line-clamp-2 text-sm text-ash-50">
                    {video.title}
                </h3>
                ...
                <div className="flex items-center gap-1.5 text-xs text-ash-300">
                    ...
                    {video.upload_date && (
                        <>
                            <span>·</span>
                            <span>{getRelativeDate(video.upload_date)}</span>
                        </>
                    )}
                </div>
            </div>
